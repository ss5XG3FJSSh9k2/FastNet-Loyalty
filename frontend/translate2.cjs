...
const reps = [
  ...
  [`<span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Maximum allowed: {stockistProfile.max_delivery_radius_km || 5.0} km</span>`, `<span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{t('Maximum allowed', 'अधिकतम अनुमत', 'সর্বাধিক অনুমোদিত')}: {stockistProfile.max_delivery_radius_km || 5.0} {t('km', 'किमी', 'কিমি')}</span>`],

  // Payout Details
  [`<h3>Payout Details</h3>`, `<h3>{t('Payout Details', 'भुगतान विवरण', 'পেআউট বিবরণ')}</h3>`],
  [`<label className="input-label">UPI ID (Primary)</label>`, `<label className="input-label">{t('UPI ID (Primary)', 'UPI आईडी (प्राथमिक)', 'UPI আইডি (প্রাথমিক)')}</label>`],
