...
content = content.replace(
  /const handleApproveKyc = async \(userId, vendorId\) => \{[\s\S]*?try\s*\{\s*const payload = \{\s*userId,\s*vendorId:[^,]+,/m,
  `const handleApproveKyc = async (userId, vendorId) => {
...
content = content.replace(/showToast\(`Approved stockist! Assigned vendor: \$\{matchingVendor\.name\}`\);/g, "showToast('Approved stockist successfully!');");

// 4. Update the buttons that trigger approval
const oldTableButtonsRegex = /<button className="btn btn-accent"[\s\S]*?✓ Approve\s*<\/button>/m;
const newTableButtons = `<button className="btn btn-accent" style={{ padding: '0.25rem 0.5rem', fontSize: '0.75rem' }} onClick={() => {
                                  const highFlags = hasHigh ? activeFlags.filter(f => f.severity === 'HIGH') : [];
                                  openKycApproveModal(u, highFlags);
                                }}>
                                  ✓ Approve
                                </button>`;
content = content.replace(oldTableButtonsRegex, newTableButtons);
...
console.log('App.jsx patched for KYC Appprove!');
